...
export function ParentAddForm() {
  ...
  function onCodeChange(v: string) {
    ...
    if (next.length === 6) {
      start(async () => {
        setCodePhase(next === '123456' ? 'verified' : 'error');
      });
    }
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-1 flex-col">
        <div className="flex flex-1 flex-col gap-4.5 px-5.5 pt-7.5">
          ...
          <div className="flex flex-col gap-2.5">
            <label htmlFor="add-name" className="text-ink-700 text-sm font-semibold">
              이름
            </label>
            <input
              id="add-name"
          ...
          {/* 휴대폰 번호 */}
          <div className="flex flex-col gap-2.5">
            <label htmlFor="add-phone" className="text-ink-700 text-sm font-semibold">
              휴대폰 번호
            </label>
          ...
          <div aria-live="polite" aria-atomic="true">
            {codePhase !== 'idle' && (
              <div className="flex flex-col gap-2.5">
                <label htmlFor="add-code" className="text-ink-700 text-sm font-semibold">
                  인증번호
                </label>
                <div
                ...
                >
                  ...
                  {codePhase === 'verified' ? (
                    ...
                    <span
                      ...
                      aria-live="off"
                      className={`ml-auto pl-2 text-sm font-semibold tabular-nums ${codePhase === 'error' ? 'text-destructive' : 'text-ink-500'}`}
                    >
                      {mm}:{ss}
                    </span>
                  )}
        ...
        <div className="px-5.5 pt-4.5 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
          <button
            ...
            aria-busy={pending}
            className="enabled:bg-primary enabled:text-primary-foreground enabled:shadow-fab disabled:bg-muted disabled:text-ink-400 focus-visible:ring-primary flex h-14 w-full items-center justify-center rounded-2xl text-base font-bold transition-colors duration-150 focus-visible:ring-2 focus-visible:outline-none enabled:hover:bg-blue-700 disabled:cursor-not-allowed"
          >
            다음
          </button>
        </div>
